...
function inject (bot, options) {
  ...
  const entitySkipMovedEvents = options.entitySkipMovedEvents ?? false
  bot.ignoreEntitiesByName = options.ignoreEntitiesByName ?? []
  bot._ignoredEntityIds = new Set()

  bot.setIgnoreEntities = (names) => {
    bot.ignoreEntitiesByName = names
    bot._ignoredEntityIds.clear()
    for (const id in bot.entities) {
      const entity = bot.entities[id]
      if (names.includes(entity.name)) {
        const numericId = Number(id)
        bot._ignoredEntityIds.add(numericId)
  ...
  bot._client.on('entity_equipment', (packet) => {
    if (bot._ignoredEntityIds.has(packet.entityId)) return
    // entity equipment
  ...
  bot._client.on('collect', (packet) => {
    if (bot._ignoredEntityIds.has(packet.collectorEntityId) || bot._ignoredEntityIds.has(packet.collectedEntityId)) return
    // collect item
  ...
  bot._client.on('spawn_entity', (packet) => {
    ...
    if (entityData && bot.ignoreEntitiesByName.includes(entityData.name)) {
      bot._ignoredEntityIds.add(packet.entityId)
      return
    }
    bot._ignoredEntityIds.delete(packet.entityId)
  ...
  bot._client.on('spawn_entity_experience_orb', (packet) => {
    if (bot.ignoreEntitiesByName.includes('experience_orb')) {
      bot._ignoredEntityIds.add(packet.entityId)
      return
    }
    bot._ignoredEntityIds.delete(packet.entityId)
  ...
  bot._client.on('entity_destroy', (packet) => {
    ...
    packet.entityIds.forEach((id) => {
      bot._ignoredEntityIds.delete(id)
  ...
  bot._client.on('rel_entity_move', (packet) => {
    ...
    if (!entitySkipMovedEvents && bot.listenerCount('entityMoved') > 0) bot.emit('entityMoved', entity)
  })
  ...
  bot._client.on('spawn_entity_weather', (packet) => {
    if (bot.ignoreEntitiesByName.includes('thunderbolt')) {
      bot._ignoredEntityIds.add(packet.entityId)
      return
    }
    bot._ignoredEntityIds.delete(packet.entityId)
  ...
  function handlePlayerInfoBitfield (packet) {
    let i = 0
    const processItems = () => {
      const batchSize = Math.min(50, packet.data.length - i)
      const endI = i + batchSize
      
      while (i < endI) {
        const item = packet.data[i]
    ...
    const processItems = () => {
      ...
      while (i < endI) {
        ...
        switch (packet.action) {
          ...
          case 'remove_player': {
            if (player) {
              if (player.entity === bot.entity) { i++; continue }
              player.entity = null
  ...
  bot._client.on('set_passengers', ({ entityId, passengers }) => {
    if (bot._ignoredEntityIds.has(entityId)) return
