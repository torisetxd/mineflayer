...
function inject (bot, { version, storageBuilder, hideErrors, sharedWorld }) {
  ...
  bot._client.on('map_chunk_bulk', (packet) => {
    let i = 0
    const processMeta = () => {
      const batchSize = Math.min(10, packet.meta.length - i)
      const endI = i + batchSize
      let offset = packet.meta.slice(0, i).reduce((sum, m) => {
        return sum + ((8192 + (packet.skyLightSent ? 2048 : 0)) * onesInShort(m.bitMap) + 2048 * onesInShort(m.bitMap) + 256)
      }, 0)

      while (i < endI) {
        const meta = packet.meta[i]
      ...
      if (i < packet.meta.length) {
        setImmediate(processMeta)
      }
    }
    processMeta()
  })
  ...
  bot._client.on('multi_block_change', (packet) => {
    ...
    const processRecords = () => {
      const batchSize = Math.min(50, packet.records.length - i)
      const endI = i + batchSize
      ...
      if (i < packet.records.length) {
        setImmediate(processRecords)
      }
    }
    processRecords()
  })
  ...
  let listener
  let listenerRemove
  const worldEventWrappers = new Map()

  function stopListenerProxy () {
    if (!bot.world) return
    if (listener) {
      bot.off('newListener', listener)
      bot.off('removeListener', listenerRemove)
    }
    for (const [event, wrapper] of worldEventWrappers) {
      bot.world.off(event, wrapper)
    }
    worldEventWrappers.clear()
  }
  ...
  function startListenerProxy () {
    ...
    for (const event of forwardedEvents) {
      const wrapper = (...args) => bot.emit(event, ...args)
      worldEventWrappers.set(event, wrapper)
      bot.world.on(event, wrapper)
    }

    const blockUpdateRegex = /blockUpdate:\(-?\d+, -?\d+, -?\d+\)/ // Escaped backslashes for regex
  ...
  bot.setWorld = (newWorld) => {
    if (!newWorld || bot.world === newWorld) return
    const oldWorld = bot.world
    stopListenerProxy()

    const blockUpdateRegex = /blockUpdate:\(-?\d+, -?\d+, -?\d+\)/ // Escaped backslashes for regex
    for (const event of bot.eventNames()) {
      if (typeof event === 'string' && blockUpdateRegex.test(event)) {
        for (const l of bot.listeners(event)) {
          if (oldWorld) oldWorld.off(event, l)
          newWorld.on(event, l)
        }
      }
    }
  ...
  async function switchWorld () {
    if (bot.world) {
      ...
      const newWorld = sharedWorld || new World(null, storageBuilder ? storageBuilder({ version: bot.version, worldName: dimensionToFolderName(dimension) }) : null).sync
      bot.setWorld(newWorld)
    }
  }
