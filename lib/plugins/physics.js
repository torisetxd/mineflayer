...
function inject (bot, { physicsEnabled, maxCatchupTicks, manualPhysics = false }) {
  ...
  function doPhysics () {
    ...
    // Optimization: If we are lagging significantly (more than 1.5s behind),
    // drop the accumulated time to avoid blocking the event loop with physics catch-up.
    // This prioritizes network handling (keep-alive) over physics simulation accuracy during lag spikes.
    if (timeAccumulator > 1.5) {
      timeAccumulator = 0
      tickPhysics(now) // Run at least one tick to ensure position packets are sent
    } else {
      while (timeAccumulator >= PHYSICS_TIMESTEP && catchupTicks < PHYSICS_CATCHUP_TICKS) {
        tickPhysics(now)
  ...
  function tickPhysics (now) {
    if (bot.world.getColumnAt(bot.entity.position) == null) return // check if chunk is unloaded
  ...
  function sendPacketPosition (position, onGround) {
    // sends data, no logic
    const emitMove = bot.listenerCount('move') > 0
    const oldPos = emitMove ? new Vec3(lastSent.x, lastSent.y, lastSent.z) : null
    ...
    if (emitMove) bot.emit('move', oldPos)
  }
  ...
  bot.physics = physics
  bot.physics.tick = (now) => tickPhysics(now || performance.now())
  bot.physics.manualPhysics = manualPhysics

  bot.physics.setEnabled = (enabled) => {
    if (bot.physics.manualPhysics === !enabled) return
    bot.physics.manualPhysics = !enabled
    if (enabled) {
  ...
  bot.on('login', () => {
    ...
    if (doPhysicsTimer === null && !bot.physics.manualPhysics) {
      lastPhysicsFrameTime = performance.now()
      doPhysicsTimer = setInterval(doPhysics, PHYSICS_INTERVAL_MS)
    }
  })
